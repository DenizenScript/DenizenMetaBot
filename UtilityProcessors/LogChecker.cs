...
namespace DenizenBot.UtilityProcessors
{
    ...
    public class LogChecker
    {
        ...
        static LogChecker()
        {
            ...
            AddReportedEntry(SUSPICIOUS_PLUGINS, $"{RED_FLAG_SYMBOL} **(Offline login authenticator plugin)**", "AuthMe", "UTitleAuth", "LoginSecurity", "nLogin", "PinAuthentication", "LockLogin", "JPremium", "FastLogin", "AmkMcAuth", "RoyalAuth", "JAuth", "AdvancedLogin", "OpeNLogin", "NexAuth", "Authy", "PasswordLogOn", "LibreLogin", "UserLogin");
        ...
        public void ProcessJavaVersion()
        {
            ...
            if (JavaVersion.StartsWith("16") || JavaVersion.StartsWith("17") || JavaVersion.StartsWith("21") || JavaVersion.StartsWith("25"))
            {
                JavaVersion = $"`{Escape(JavaVersion)}` {GREEN_CHECK_MARK_SYMBOL}";
                return;
            }
            JavaVersion = $"`{Escape(JavaVersion)}` {WARNING_SYMBOL} - Only Java versions 25, 21, or 17 are fully supported";
        }
